<template>
	<view ref="node" name="LyTreeNode" v-show="node.visible" class="ly-tree-node" :class="{
		...
		<view class="ly-tree-node__content" :class="{
			...
			<block v-if="node.label instanceof Array">
				<view v-for="(item,index) in node.label" :key="index" class="ly-tree-node__label" style="flex: 1;">
					<image v-if="item && typeof(item)=='string' && item.indexOf('http') !== -1 " :src="item" mode="aspectFill" class="suoluetu"></image>
					<text v-else>{{item}}</text>
				</view>
			</block>
			<text class="ly-tree-node__label" v-else>
				{{node.label}}
			</text>
			...
			<block v-if="showActions">
				<view>
					<text>【</text>
						<text @tap.stop="handleExtButtonClick('edit')" class="ly-tree-node__extbutton">修改</text>
					<text>】</text>
				</view>
				<view>
					<text>【</text>
		...
		<view v-if="!renderAfterExpand || childNodeRendered" v-show="expanded" class="ly-tree-node__children"
			...
			<ly-tree-node v-for="cNodeId in node.childNodesId" :nodeId="cNodeId"
				...
				:showActions="showActions" :check-only-leaf="checkOnlyLeaf" :key="getNodeKey(cNodeId)" :indent="indent"
