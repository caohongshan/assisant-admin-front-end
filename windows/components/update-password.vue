		...
		<view class="uni-container">
			<uni-forms ref="resetPwdForm" v-model="password" :rules="rules" @submit="submit">
				...
				<uni-forms-item :label="$t('updatePwd.field.newPassword')" name="newPassword" labelWidth="85">
					<input class="uni-input-border" :password="showPassword"
						:placeholder="$t('updatePwd.field.newPassword')" v-model="password.newPassword" />
					<!-- <text class="uni-icon-password-eye pointer" :class="[!showPassword ? 'uni-eye-active' : '']" @click="changePassword">&#xe568;</text> -->
				</uni-forms-item>

				<uni-forms-item :label="$t('updatePwd.field.passwordConfirmation')" name="passwordConfirmation"
					labelWidth="85" :errorMessage="errorMessage">
					<input @confirm="submitForm" class="uni-input-border" :password="showPasswordAgain"
						:placeholder="$t('updatePwd.field.passwordConfirmation')"
				...
				<view class="uni-button-group pointer">
					...
					<button v-if="hasBackButton" class="uni-button login-button-width" type="default"
						:disabled="isLoading" @click="back">{{$t('updatePwd.button.back')}}</button>
				</view>
			</uni-forms>
		</view>
	</view>
