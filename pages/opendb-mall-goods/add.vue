			...
			<view class="ti-flex" style="align-items: baseline;">
				<view class="">
					...
					<!-- <uni-forms-item name="goods_thumb" label="缩略图">
						<cloud-image name="src" v-model="formData.goods_thumb" :value="formData.goods_thumb" />
						<view class="">
							<text class="colorBlue">尺寸600x600，支持jpg/png</text>
						</view>
					</uni-forms-item> -->
					...
					<uni-forms-item name="goods_banner" label="布局大图">
						...
						<view class="">
							<text class="colorBlue">尺寸710x340，支持jpg/png，商品推荐时可用</text>
						</view>
					</uni-forms-item>
					...
					<block v-if="$hasRole('admin')">
						...
						<uni-forms-item name="rebate_score_rate" label="积分返佣">
							<uni-easyinput placeholder="积分返佣，多级别用英文逗号隔开" v-model="formData.rebate_score_rate"
								trim="both" />
							<view class="margin-top-20">
								<text class="colorBlue">大于1，表示返现金，单位为分;小于等于1，表示返商品金额比例，范围0.01~1</text>
							</view>
						</uni-forms-item>
					...
					<uni-forms-item name="seller_note" label="商家备注" labelPosition="top">
				...
				<view class="margin-left-80">
					<uni-forms-item name="category_id" label="分类" required>
						<view class="" style="width: 480px;">
							...
							<tian-mall-categories-admin v-model="formData.category_id" :shopid="adminShopId">
	...
	export default {
			...
			return {
				...
				rules: {
					...getValidator(["category_id", "goods_sn", "name", "price", "market_price", "keywords", "goods_desc",
						...
						"rebate_score_rate", "rebate_money_rate", "use_score", "group_buying", "group_buying_discount"
					])
				},
			}
		},
		...
		methods: {
			...
			changeSkuName(e) {
				...
				//生成规格信息
				console.log(this.skuOriginList, this.formData)
			},
			addSku(index, index2) {
			...
			submit() {
				...
				this.$refs.form.submit().then((res) => {
					res.add_date = Date.now();
					res.dcloud_appid = this.adminShopDcloudAppid;
					...
					if (this.formData.skus && this.formData.skus.length > 0) {
						...
						skus.map(sku => {
							...
							if (minPrice == -1 || minPrice > sku.price) {
								...
								// res.market_price = sku.market_price ? Math.round(sku.market_price * 100) : 0;
								minPrice = res.price;
							}
							//更新库存类型
						...
						res.remain_count = parseInt(res.remain_count);
			...
			submitForm(value, skus) {
				// 使用 clientDB 提交数据
				this.$requestApp("mall/goods/add", {value}).then(result => {
					console.log("submit result", result)
					this.formDataId = result._id;
					uni.showToast({
						title: '修改成功'
					})
					this.saveSkus(skus);
			...
			getDetail(id) {
				...
				db.collection(dbCollectionName).doc(id).field(
					'category_id,goods_sn,name,price,market_price,keywords,goods_desc,goods_thumb,goods_banner_imgs,remain_count,month_sell_count,total_sell_count,comment_count,is_real,is_on_sale,is_alone_sale,is_best,is_new,is_hot,is_vip,add_date,last_modify_date,sku_name,seller_note'
				).get().then((res) => {
			...
			getSkusData(id) {
				...
				db.collection("opendb-mall-sku").where({
					...
					result.data.forEach(sku => {
						...
						sku.sku_name.split("&gt;").map((n, index) => {
